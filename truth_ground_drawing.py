...
def draw_predict_box(img_ground_predict,predict_txt):
    file = open(predict_txt)
    while 1:
        line = file.readline()
        if not line:
            break
        location=line.split(" ")
        xmin=int(float(location[2]))
        ymin=int(float(location[3]))
        xmax = xmin+int(float(location[4]))
        ymax = ymin+int(float(location[5]))
        cv2.rectangle(img_ground_predict, (xmin, ymin), (xmax, ymax), (0, 255, 255), 2)
    return img_ground_predict
...
def readTXT(filename):
    ...
    while 1:
        ...
        if not os.path.exists("./"+labels[0]):
            ...
            continue
        img_truth=cv2.imread("./"+labels[0])
        img_predict=cv2.imread("./"+labels[0])
        imgfile_name=labels[0].split("/")[-1]
        predict_txt="/home/tylor/nvme_guazai/eval_file/prediction/"+imgfile_name[:-3]+"txt"
        # print(predict_txt)
        if not os.path.exists(predict_txt):
            print("txt not exit: pass")
            predict_txt="empty.txt"
            continue
        img_predict=draw_predict_box(img_predict,predict_txt)
        # draw_predict_box()
        ...
        for i in range(len(labels)-1):
            ...
            # print(len(parameters),parameters[4])
            if len(parameters)>1:
                if parameters[4]=="10":
                    # print("####################")
                    cv2.rectangle(img_truth, (int(float(parameters[0])), int(float(parameters[1]))), (int(float(parameters[2])), int(float(parameters[3]))), (255, 255, 0), 2)
        img_predict = cv2.resize(img_predict, (640, 480))
        img_truth = cv2.resize(img_truth, (640, 480))
        image = np.vstack((img_predict, img_truth))
        cv2.imwrite("./ground_predict_pic/"+imgfile_name, image)
